...
## Cross-sectional data and idividual dummies
...
## Panel data data and idividual dummies {.smaller}
...
-    NOT ZERO correlation between FE and regressors: $Cov(\delta_{i},{X}_{it}) \neq 0$
...
# Panel regression: motivation for a panel regression
...
3.  Random Effect;
    ...
    -   `Hausman test`, `Chamberlain test`, `Angrist and Newey` on RE consistency against FE;
...
```{r}
rice_re <- 
  plm(goutput ~ size + seed + urea + totlabor, 
      rice_dta_p, 
      model = "random", 
      effect = "individual")

compare_models(rice_pooled, rice_fe, rice_re, style = "se_p") %>% 
  print(digits = 3)
...
### Step 5. Robust and clustered standard errors
